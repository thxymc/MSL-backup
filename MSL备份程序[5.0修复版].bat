...
REM #严禁二次包装出售
REM #脚本源码已公开，非付费工具
REM #本工具集成7zip压缩工具，以及时间计算器
REM #联系qq群:1145888872  联系邮箱:thxymc@outlook.com
REM #GitHub:https://github.com/thxymc/MSL-backup

REM #变量
set a=0
set title=MSL备份程序 5.0修复版
title %title% [备份次数%a%]
REM #文件夹检测
if exist MSL (
goto test
...
echo ---------------------------------------------------
echo                 使用协议
echo 1.1 若你在程序协议里填写true则代表你同意此协议
...
echo   ╚╝  ╚╝ ╚╝╚═╝╚═╝  ╚╝  ╚╩╩╩══╩╩══╩══╩╩╩╝
echo ---------------------------------------------------
echo 注意！！！需要备份的文件/文件夹的路径中不能有中文"
echo 注意！！！需要备份的文件/文件夹的路径中不能有中文"
echo 也就是说下面不能输入中文，只能是英文或者数字，被备份的文件夹内可以使用中文"
echo 否则会出现乱码的情况"
echo 实例[文件夹备份格式]: D:\thxy\  C:\aaa\qqq\  e:\Minecraft\server\
echo 实例[文件备份格式]: D:\thxy\MSL.bat  C:\aaa\qqq\MSL.bat  e:\Minecraft\server\MSL.bat
...
echo 脚本名字:MSL备份程序
echo 当前版本:5.0修复版
echo 本bat脚本由天河星雨服务器腐竹小姐姐编写
echo 本脚本版权归天河星雨工作室所有
echo 严禁二次包装出售
echo 脚本源码已公开，非付费工具
echo 本工具集成7zip压缩工具，以及时间计算器
echo 联系qq群:1145888872  联系邮箱:thxymc@outlook.com
echo GitHub:https://github.com/thxymc/MSL-backup
echo ---------------------------------------------------
timeout /nobreak /t 3
:menu1
echo %AD1%
echo 请选择执行命令
echo [输入1开始自动备份],[输入2修改备份时间],[输入3修改备份信息],[输入4手动进行备份],[输入5查看脚本协议]
set /P AA=
if "%AA%"=="1" set run2=a
if "%AA%"=="4" set run2=b
if "%AA%"=="1" goto run1
